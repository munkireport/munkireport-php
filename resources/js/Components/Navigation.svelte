    ...
    {#if $isLoading}{:else}
        <Collapse {isOpen} navbar expand="md" on:update={handleUpdate}>
                ...
                <Dropdown nav inNavbar>
                    ...
                    <DropdownMenu>
                        {#each $page.props.reports as report}
                            <DropdownItem class="{report.class}"
                                          href="{report.url}">{$i18n.t(report.i18n)}</DropdownItem>
                        {/each}
                    </DropdownMenu>
                </Dropdown>
                ...
                <Dropdown nav inNavbar>
                    ...
                    <DropdownMenu>
                        {#each $page.props.listings as listing}
                            <DropdownItem class="{listing.class}"
                                          href="{listing.url}">{$i18n.t(listing.i18n)}</DropdownItem>
                        {/each}
                    </DropdownMenu>
                </Dropdown>
            ...
            <Nav navbar class="ml-auto">
                <Form inline>
                    <InputGroup>
                        <Input type="search" placeholder="Search" aria-label="Search" aria-haspopup="true"
                               aria-expanded="false"/>
