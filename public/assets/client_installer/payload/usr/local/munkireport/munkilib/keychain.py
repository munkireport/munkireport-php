...
DEFAULT_KEYCHAIN_NAME = 'munkireport.keychain'
DEFAULT_KEYCHAIN_PASSWORD = 'munkireport'
KEYCHAIN_DIRECTORY = os.path.join('/usr/local/munkireport/keychains')
...
def get_munki_client_cert_info():
    ...
    if prefs.pref('UseClientCertificate'):
        ...
        if not cert_info['client_cert_path']:
            for name in ['cert.pem', 'client.pem', 'munkireport.pem']:
...
class MunkiKeychain(object):
    ...
    def __init__(self):
        ...
        Unlocks the munkireport.keychain if it exists.
        Makes sure the munkireport.keychain is in the search list.
        Creates a new client keychain if needed.'''
        # add_ca_certs_to_system_keychain()
        # ^^ Commented out until more debugging can be done
