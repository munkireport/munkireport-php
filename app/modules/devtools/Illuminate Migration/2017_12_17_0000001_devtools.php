...
class Devtools extends Migration
{
    public function up()
    {
        ...
        $capsule::schema()->create('devtools', function (Blueprint $table) {
            ...
            $table->index('cli_tools', 'devtools_cli_tools');
            $table->index('dashcode_version', 'devtools_dashcode_version');
            $table->index('devtools_version', 'devtools_devtools_version');
            $table->index('instruments_version', 'devtools_instruments_version');
            $table->index('interface_builder_version', 'devtools_interface_builder_version');
            $table->index('xcode_version', 'devtools_xcode_version');
            $table->index('xquartz', 'devtools_xquartz');
        });
    }
