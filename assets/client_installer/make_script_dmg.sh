...
printf "\nCopying files\n"
for i in preflight postflight report_broken_client reportcommon; do
...
md5_reportcommon=`md5 ${tmproot}/reportcommon | awk '{print $NF}'`
...
<dict>
	...
	<array>
		...
		<dict>
			<key>md5checksum</key>
			<string>${md5_reportcommon}</string>
			<key>path</key>
			<string>/usr/local/munki/reportcommon</string>
			<key>type</key>
			<string>file</string>
		</dict>
	</array>
